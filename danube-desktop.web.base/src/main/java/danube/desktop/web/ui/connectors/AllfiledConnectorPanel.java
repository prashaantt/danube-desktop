...
public class AllfiledConnectorPanel extends Panel implements XdiGraphListener, ExternalCallReceiver {
	...
	public void init() {
		...
		this.allfiledApi = new AllfiledApi("1191", "TC2Sgky11WcpeyFYdhGVlNhuOP2LqjSmwJTHLKhQXjGvaedDOG");
	...
	private void onConnectAllfiledActionPerformed(ActionEvent e) {
		...
		try {
			...
			DesktopApplication.getApp().enqueueCommand(new BrowserRedirectCommand(this.allfiledApi.startOAuth(request, redirectUri, userXri)));
			return;
		} catch (Exception ex) {
	...
	public void onExternalCall(DesktopApplication pdsApplication, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (request.getParameter("code") != null) {
			...
			XRI3Segment userXri = AllfiledConnectorPanel.this.endpoint.getCanonical();

			try {

				AllfiledConnectorPanel.this.allfiledApi.checkState(request, userXri);

				final String accessToken = AllfiledConnectorPanel.this.allfiledApi.exchangeCodeForAccessToken(request);
